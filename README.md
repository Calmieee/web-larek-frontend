...
5. `basket:open` - открытие корзины <br>
    * Передаёт данные  из `AppState.basket` и `appState.getTotalPrice()`(через посредника) в класс `BasketView` для последующей отрисовки окна с корзиной(передача элементов не напрямую в класс Modal, а также через посредника в слое presenter)
...
2. Класс `EventEmmite`r Реализует паттерн «наблюдатель» и позволяет подписываться на события и уведомлять подписчиков о наступлении события. Конструктор принимает экземпляр объекта Map и записывает его в свойство _events.
